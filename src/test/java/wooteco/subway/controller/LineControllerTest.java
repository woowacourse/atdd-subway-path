...
class LineControllerTest {
    ...
    void createLineNameBlank(String lineName) throws Exception {
        ...
        mockMvc.perform(request)
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("$.message").value("지하철 노선의 이름을 입력해주세요."));
    }
    ...
    void createLineColorBlank(String lineColor) throws Exception {
        ...
        mockMvc.perform(request)
                .andExpect(status().isBadRequest())
                .andExpect(MockMvcResultMatchers.jsonPath("$.message").value("지하철 노선의 색상을 선택해주세요."));
    }
    ...
    @ParameterizedTest
    @ValueSource(strings = {"null", "\"\"", "\" \""})
    @DisplayName("지하철 노선 수정 시 이름에 null 또는 공백을 입력할 경우 예외가 발생한다.")
    void updateLineNameBlank(String lineName) throws Exception {
        // given
        RequestBuilder request = MockMvcRequestBuilders
                .post("/lines")
                .content("{\"name\": " + lineName + ", \"color\": \"bg-red-600\", \"upStationId\": 1, \"downStationId\": 2, \"distance\": 10}")
    ...
    @ParameterizedTest
    @ValueSource(strings = {"null", "\"\"", "\" \""})
    @DisplayName("지하철 노선 수정 시 노선 색상에 null 또는 빈값을 입력하면 예외가 발생한다.")
    void updateLineColorBlank(String lineColor) throws Exception {
        // given
        RequestBuilder request = MockMvcRequestBuilders
                .post("/lines")
                .content("{\"name\": \"신분당선\", \"color\": " + lineColor + ", \"upStationId\": 1, \"downStationId\": 2, \"distance\": 10}")
