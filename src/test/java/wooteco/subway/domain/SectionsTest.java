    ...
    Stream<DynamicTest> dynamicTestStreamNormalCase() {
        return Stream.of(
                DynamicTest.dynamicTest("상행 종점인 구간을 추가한다.", () -> {
                    Sections sections = new Sections(new Section(0, new Line(), 1L, 2L, 7));
                    sections.add(new Section(0, new Line(), 3L, 1L, 5));

                    assertThat(sections.getStations().size()).isEqualTo(3);
                }),
                DynamicTest.dynamicTest("하행 종점인 구간을 추가한다.", () -> {
                    Sections sections = new Sections(new Section(0, new Line(), 1L, 2L, 7));
                    sections.add(new Section(0, new Line(), 2L, 3L, 5));

                    assertThat(sections.getStations().size()).isEqualTo(3);
                }),
                DynamicTest.dynamicTest("갈래길을 방지한다.", () -> {
                    Sections sections = new Sections(new Section(0, new Line(), 1L, 2L, 7));
                    sections.add(new Section(0, new Line(), 1L, 3L, 5));
