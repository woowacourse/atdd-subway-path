...
class FareTest {
    ...
    void calculateFare(int distance, int expectedFare) {
        Fare fare = Fare.from(distance, 0, 20);
    ...
    void extraFareTest(int distance, int extraFare, int expectedFare) {
        Fare fare = Fare.from(distance, extraFare, 20);

        assertThat(fare.getValue()).isEqualTo(expectedFare);
    }

    @Test
    @DisplayName("연령별 할인 정책이 적용된 요금을 계산한다.")
    void discountFareTest() {
        Fare fare = Fare.from(55, 200, 18);

        assertThat(fare.getValue()).isEqualTo(1600);
    }
}
