...
public class LineAcceptanceTest extends AcceptanceTest {
    ...
    private StationResponse downStationRequestDto;
    ...
    public void setUp() {
        ...
        lineRequest1 = new LineRequest("신분당선", "bg-red-600", 강남역.getId(), downStationRequestDto.getId(), 10);
        lineRequest2 = new LineRequest("구신분당선", "bg-red-600", 강남역.getId(), downStationRequestDto.getId(), 15);
    }
    ...
    public static LineResponse 지하철_노선_등록되어_있음(String name, String color, StationResponse upStationResponse, StationResponse downStationResponse, int distance) {
        LineRequest lineRequest = new LineRequest(name, color, upStationResponse.getId(), downStationResponse.getId(), distance);
    ...
    public static ExtractableResponse<Response> 지하철_노선_조회_요청(LineResponse lineResponse) {
        return RestAssured
                .given().log().all()
                .accept(MediaType.APPLICATION_JSON_VALUE)
                .when().get("/api/lines/{lineId}", lineResponse.getId())
                .then().log().all()
                .extract();
    }
    ...
    public static ExtractableResponse<Response> 지하철_노선_수정_요청(LineResponse lineResponse, LineRequest lineRequest) {
        ...
        return RestAssured
                ...
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .body(lineRequest)
                .when().put("/api/lines/" + lineResponse.getId())
                .then().log().all()
                .extract();
    }
    ...
    public static void 지하철_노선_응답됨(ExtractableResponse<Response> response, LineResponse lineResponse) {
        assertThat(response.statusCode()).isEqualTo(HttpStatus.OK.value());
        LineResponse resultLineResponse = response.as(LineResponse.class);
        assertThat(resultLineResponse.getId()).isEqualTo(lineResponse.getId());
    }

    public static void 지하철_노선_목록_포함됨(ExtractableResponse<Response> response, List<LineResponse> createdLineResponses) {
