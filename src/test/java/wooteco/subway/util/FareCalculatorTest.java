...
class FareCalculatorTest {
    ...
    public void calculateFare() {
        assertThat(FareCalculator.calculate(new Distance(5), Age.from(20), 0)).isEqualTo(1250);
    }
    ...
    public void calculateFareByDistance(int distance, int expectedFare) {
        assertThat(FareCalculator.calculate(new Distance(distance), Age.from(20), 0)).isEqualTo(expectedFare);
    }
    ...
    public void discountToTeenager(int distance, int expectedFare) {
        assertThat(FareCalculator.calculate(new Distance(distance), Age.from(15), 0)).isEqualTo(expectedFare);
    }
    ...
    public void discountChildren(int distance, int expectedFare) {
        assertThat(FareCalculator.calculate(new Distance(distance), Age.from(8), 0)).isEqualTo(expectedFare);
    }

    @Test
    @DisplayName("추가 금액이 있는 경우 함께 더한다.")
    public void extraFare() {
        assertThat(FareCalculator.calculate(new Distance(5), Age.from(20), 100)).isEqualTo(1350);
    }

    @Test
    @DisplayName("영유아의 경우 0원이다.")
    void baby() {
        assertThat(FareCalculator.calculate(new Distance(100), Age.from(3), 500)).isEqualTo(0);
    }
