...
class SectionAcceptanceTest extends AcceptanceTest {
    ...
    Stream<DynamicTest> sectionTest() {
        ...
        post(LINES_URI, toJson(new LineRequest("2호선", "bg-600-green", 2L, 4L, 20L)));

        return Stream.of(
                dynamicTest("삼성-역삼 에 성수-삼성을 추가하면 성수-삼성-역삼 이 된다", () -> {
                    // given
                    final String body = toJson(new SectionRequest(1L, 2L, 10L));
                ...
                dynamicTest("성수-삼성-역삼 에 역삼-강남 을 추가하면 성수-삼성-역삼-강남 이 된다", () -> {
                    // given
                    final String body = toJson(new SectionRequest(4L, 5L, 10L));
                ...
                dynamicTest("성수-삼성-역삼-강남 에 삼성-선릉 을 추가하면 성수-삼성-선릉-역삼-강남 이 된다", () -> {
                    // given
                    final String body = toJson(new SectionRequest(2L, 3L, 5L));
                ...
                dynamicTest("성수-선릉-역삼-강남 에 거리가 초과되면 구간 추가가 실패한다", () -> {
                    // given
                    final String body = toJson(new SectionRequest(1L, 2L, 50L));
