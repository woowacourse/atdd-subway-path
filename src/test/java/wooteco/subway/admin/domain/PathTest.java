...
class PathTest {
	...
	void findShortestPathWithSameSourceAndDestination(PathSearchType type) {
		path = new Path(Arrays.asList(lineOne, lineTwo));
		assertThatThrownBy(() -> path.findShortestPath(1L, 1L, type)).isInstanceOf(
	...
	void findShortestPathBetweenDisconnectedSourceAndDestination(PathSearchType type) {
		...
		path = new Path(Arrays.asList(lineOne, lineTwo));
		assertThatThrownBy(() -> path.findShortestPath(1L, 100L, type)).isInstanceOf(
			IllegalArgumentException.class);
	}

	@DisplayName("최단 경로 조회시 출발역이나 도착역이 없는 경우, 예외를 발생시킨다.")
	@ParameterizedTest
	@CsvSource({"DISTANCE,100,1", "DISTANCE,1,100", "DISTANCE,100,100", "DURATION,100,1", "DURATION,1,100",
		"DURATION,100,100"})
	void findShortestDurationPathWithNonExistentSourceOrDestination(PathSearchType type, Long source, Long target) {
	...
	void findShortestPathWithNullSourceOrDestination(Long source, Long target, PathSearchType type) {
		path = new Path(Collections.singletonList(lineOne));
		assertThatThrownBy(() -> path.findShortestPath(source, target, type)).isInstanceOf(
			IllegalArgumentException.class);
	}
