...
export const optionTemplate = (line) =>
	`<option data-line-id=${line.id}>${line.name}</option>`;
...
export const subwayLinesItemTemplate = (line) => {
	...
	return `<div class="inline-block w-1/2 px-2">
            <div class="rounded-sm w-full slider-list">
              <div class="border ${line.color} lint-title px-4 py-1" data-line-id="${line.id}">${line.name}</div>
...
export const initNavigation = () => {
	document
	.querySelector('body')
	.insertAdjacentHTML('afterBegin', navTemplate);
};
