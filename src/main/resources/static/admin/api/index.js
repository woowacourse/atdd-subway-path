...
const api = (() => {
  ...
  const line = {
    ...
    getAllDetail() {
      return requestWithJsonData(`/paths/lines/detail`)
    },
    addLineStation(lineId, lineStationCreateRequestView) {
      return request(`/paths/lines/${lineId}/stations`, METHOD.POST(lineStationCreateRequestView))
    },
    deleteLineStation(lineId, stationId) {
      return request(`/paths/lines/${lineId}/stations/${stationId}`, METHOD.DELETE())
    }
  };
