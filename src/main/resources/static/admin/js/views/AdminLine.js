...
function AdminLine() {
	...
	const updateSubwayLineList = (res) => {
		changeInfo.target
		.querySelector('span')
		.classList.remove(changeInfo.beforeColor);
	...
	const onSelectSubwayLine = (event) => {
		...
		const isSelectSubwayLine = $target.classList.contains(
			'subway-line-item',
		);
	...
	const onCreateSubwayLine = () => {
		...
		api.line.create(newSubwayLine).then((res) => settingLineList(201, res));
		subwayLineModal.toggle();
	};
	...
	const onUpdateSubwayLine = () => {
		let newSubwayLine = subwayLineModal.makeFrom();
		api.line
		.update('/' + subwayLineModal.subwayLineId(), newSubwayLine)
		.then((res) => settingLineList(200, res));
		subwayLineModal.toggle();
	};
