...
function AdminEdge() {
	...
	const onRemoveStationHandler = (event) => {
		...
		const lineId = $target
		.closest('.slider-list')
		.querySelector('.lint-title').dataset.lineId;
		const stationId = $target.closest('.list-item').dataset.stationId;

		api.line
		.delete(`/${lineId}/stations/${stationId}`)
	...
	const onAddLineStationHandler = (event) => {
		...
		const preStationName = document.querySelector('#depart-station-name')
			.value;
		const stationName = document.querySelector('#arrival-station-name')
			.value;
