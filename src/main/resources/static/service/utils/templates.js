...
export const searchResultTemplate = result => {
  const lastIndex = result.stationResponses.length - 1
  const pathResultTemplate = result.stationResponses.map((station, index) => pathStationTemplate(
    station.name,
    index,
    lastIndex)).join('')
