...
public class DataLoader implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        Line 신분당선 = lineDao.create(new Line("신분당선", "red lighten-1"));
        sectionDao.save(new Section(신분당선, 강남역, 판교역, 공통거리));
        sectionDao.save(new Section(신분당선, 판교역, 정자역, 공통거리));

        Line 이호선 = lineDao.create(new Line("2호선", "green lighten-1"));
        sectionDao.save(new Section(이호선, 강남역, 역삼역, 공통거리));
        sectionDao.save(new Section(이호선, 역삼역, 잠실역, 공통거리));
