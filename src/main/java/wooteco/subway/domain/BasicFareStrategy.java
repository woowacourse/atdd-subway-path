...
public class BasicFareStrategy implements FareStrategy {
    ...
    private static final double FIRST_OVER_DISTANCE_STANDARD = 5;
    private static final double SECOND_OVER_DISTANCE_STANDARD = 8;
    ...
    private int calculateOverFromTenToFifty(int distance) {
        return (int) ((Math.ceil(distance / FIRST_OVER_DISTANCE_STANDARD) * OVER_FARE_STANDARD));
    }

    private int calculateOverFifty(int overDistance) {
        return (int) ((Math.ceil(overDistance / SECOND_OVER_DISTANCE_STANDARD) * OVER_FARE_STANDARD));
    }
}
