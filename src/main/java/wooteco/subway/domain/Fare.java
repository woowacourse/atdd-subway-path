...
public class Fare {
    ...
    public int calculate() {
        ...
        if (distance.isLessThanOrEqual(DISTANCE_OF_OVER_FARE)) {
            return BASIC_FARE + calculateOverFare(STANDARD_DISTANCE_OF_OVER_FARE);
        }
        return BASIC_FARE + calculateOverFare(MAX_STANDARD_DISTANCE_OF_OVER_FARE);
    }

    private int calculateOverFare(int standardDistance) {
        return (int) ((Math.ceil((distance.getValue() - DISTANCE_OF_BASIC_FARE - 1) / standardDistance) + 1)
                * STANDARD_OF_OVER_FARE);
    }
}
