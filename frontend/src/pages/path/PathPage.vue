...
export default {
  ...
  methods: {
    ...
    async onSearchResult() {
      try {
        const pathResult_response = await fetch(`http://localhost:8080/paths?source=${this.path.source}&target=${this.path.target}`);
        this.pathResult = await pathResult_response.json();
      } catch (e) {
