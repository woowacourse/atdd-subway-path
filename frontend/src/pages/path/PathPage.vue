    ...
    async onSearchResult() {
      try {
        const stationResponse = await fetch(
            `http://localhost:8080/paths?source=${this.path.source}&target=${this.path.target}`);
        if (!stationResponse.ok) {
          const error = await stationResponse.json();
          const errorMessage = error.errorMessage
          this.showSnackbar(errorMessage);
          return;
        }

        this.pathResult = await stationResponse.json()
      } catch (e) {
