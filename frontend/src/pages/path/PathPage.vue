...
export default {
  ...
  methods: {
    ...
    async onSearchResult() {
      try {
        const sourceStation = this.path.source;
        const targetStation = this.path.target;
        const response = await fetch("http://localhost:8080/paths?source=" + sourceStation + "&target=" + targetStation);
