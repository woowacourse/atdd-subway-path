...
import { stationApiService } from "../../api/modules/station";
import { pathApiService } from "../../api/modules/path";
...
export default {
  ...
  methods: {
    ...
    async onSearchResult() {
      try {
        this.pathResult = await pathApiService.get(this.path);
      } catch (e) {
    ...
    async initAllStationsView() {
      try {
        const stations = await stationApiService.getAll();
    ...
    async onSearchMinimumDurationType() {
      try {
        // TODO 최소 시간을 검색하는 API를 추가해주세요. 타입 옵션 필요
        this.pathResultByMinimumDuration = await pathApiService.get(this.path);
      } catch (e) {
        this.showSnackbar(SNACKBAR_MESSAGES.COMMON.FAIL);
        throw new Error(e);
      }
