...
import {requestPost, requestGet} from "@/utils/fetcher";
...
export default {
  ...
  methods: {
    ...
    async initLineStationsView() {
      try {
        this.selectedLine = await requestGet(`/lines/${this.sectionForm.lineId}`, {});
    ...
    async onCreateSection() {
      ...
      try {
        await requestPost(`/lines/${this.selectedLine.id}/sections`, {
