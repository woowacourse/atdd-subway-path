...
- 2단계 요금 요구 사항
  - [x] 노선 별 추가 요금
    ...
    - [x] 추가 요금이 있는 노선을 이용하면 가장 비싼 노선의 추가요금만 더 한다.
  - [x] 연령 별 요금 할인
    - [x] 연령 별 요금 할인은 거리 별 추가 요금 + 노선 별 추가 요금을 더한 이후 적용한다.
    - [x] 청소년인 경우 운임에서 350원을 공제한 금액에서 20%을 할인한 금액을 적용한다.
    - [x] 청소년은 13 <= 나이 < 19 인 경우를 의미한다.
    - [x] 어린이 인 경우 운임에서 350원을 공제한 금액에서 50%을 할인한 금액을 적용한다.
    - [x] 어린이는 6 <= 나이 < 13 인 경우를 의미한다.
### API
...
Feature: 경로 관리 기능
  ...
  Scenario: 경로를 조회한다.
    ...
    when 상계 하계를 종점으로 가지고 거리가 5, 추가 요금이 100원인 7호선을 등록 요청한다.
    when 상계 중계를 종점으로 가지고 거리가 5, 추가 요금이 200원인 6호선을 등록 요청한다.
